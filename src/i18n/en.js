export default {
  common: {
    confirm: 'Confirm',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    search: 'Search',
    refresh: 'Refresh',
    export: 'Export',
    back: 'Back',
    close: 'Close',
    clear: 'Clear',
    submit: 'Submit',
    login: 'Login',
    logout: 'Logout',
    all: 'All',
    status: 'Status',
    actions: 'Actions',
    price: 'Price',
    quantity: 'Qty',
    total: 'Total',
    name: 'Name',
    category: 'Category',
    remark: 'Remark',
    yes: 'Yes',
    no: 'No',
    success: 'Operation successful',
    error: 'Operation failed',
    confirmDelete: 'Confirm delete? This action cannot be undone',
    loading: 'Loading...',
    noData: 'No data',
    dateRange: 'Date Range',
    startDate: 'Start Date',
    endDate: 'End Date',
    filter: 'Filter',
    reset: 'Reset',
    id: 'ID',
    required: 'Required',
    server: 'Server',
    serverUrl: 'Server URL',
    serverError: 'Server connection failed:',
    loadError: 'Failed to load: {msg}',
    dbInfo: 'Database Info',
    dbSize: 'DB Size',
    bakSize: 'Backup Size',
    backupNow: 'Backup Now',
    backupSuccess: 'Backup completed',
    restoreNow: 'Restore',
    restoreConfirm: 'Confirm restore? Current data will be overwritten.',
    restoreSuccess: 'Restored from backup. Please restart the server.',
    noBackup: 'No backup file',
    serverUrlRequired: 'Please enter server address',
  },

  errors: {
    // Login / Auth
    not_logged_in: 'Not logged in',
    permission_denied: 'Permission denied',
    account_disabled: 'Account has been disabled',
    session_kicked: 'This account was logged in on another device. Please sign in again.',
    session_expired: 'Session expired',
    login_rate_limited: 'Too many login attempts. Please try again in 5 minutes.',
    login_fields_required: 'Please enter employee ID and password',
    login_failed: 'Incorrect employee ID or password',
    ws_auth_failed: 'Authentication failed',
    // Order operations
    order_table_required: 'Please select a table and add dishes',
    table_conflict: 'This table already has a pending order. Please complete checkout first.',
    order_amount_required: 'Order amount is missing',
    order_not_found: 'Order not found',
    order_draft_only: 'Only draft orders can be submitted',
    order_id_invalid: 'Invalid order ID',
    order_must_be_pending: 'Can only add items to pending orders',
    order_not_editable: 'This order cannot be modified in its current state',
    order_not_pending: 'This order is not pending',
    order_already_completed: 'This order is already completed and cannot be cancelled',
    order_not_completed: 'This order is not completed and cannot be reopened',
    items_required: 'Please select dishes to add',
    item_id_required: 'Item ID is required',
    item_not_found: 'Item not found',
    item_already_cancelled: 'This item has already been cancelled',
    no_printable_items: 'No printable items',
    // System
    print_failed: 'Print failed',
    backup_name_invalid: 'Invalid backup file name',
    backup_not_found: 'Specified backup file not found',
    backup_file_missing: 'Backup file not found',
    restore_failed: 'Restore failed. Please check backup integrity.',
    server_error: 'Internal server error',
    request_timeout: 'Request timed out. Please check your network connection.',
  },

  offline: {
    pendingSync: '{count} offline orders to sync',
    syncNow: 'Sync Now',
    syncResult: '{ok} offline orders synced',
    syncResultFail: ', {fail} failed',
    syncError: 'Sync failed',
    queuedDraft: 'Saved to offline queue. Will sync when online.',
    queuedSubmit: 'Order queued. Will submit when online.',
    menuLoaded: 'Menu loaded from offline cache',
  },

  currency: {
    symbol: 'MT',
    code: 'MZN',
    format: (v) => `${Number(v).toFixed(2)} MT`,
  },

  nav: {
    order: 'Order System',
    admin: 'Management',
    pendingOrders: 'Pending Orders',
    menuManage: 'Menu Management',
    orderHistory: 'Order History',
    reports: 'Reports',
    employeeManage: 'Employees',
  },

  login: {
    title: 'Restaurant Management',
    employeeNo: 'Employee ID',
    password: 'Password',
    loginBtn: 'Login',
    hint: 'Default ID: admin / 123456',
    success: 'Login successful',
    error: 'Incorrect ID or password',
    accessDenied: 'Only admins or cashiers can access the management system',
    kickedOut: 'This account has been logged in on another device. Please sign in again.',
    placeholderUser: 'Enter employee ID',
    placeholderPass: 'Enter password',
    serverLabel: 'Server',
    serverPlaceholder: 'http://192.168.x.x:3000',
    serverHint: 'Emulator: use http://10.0.2.2:3000 | Real device: use PC LAN IP',
    checking: 'Checking server…',
    connected: 'Server connected',
    connectFail: 'Connection failed, please check server address',
  },

  order: {
    title: 'Order System',
    waiterLabel: 'Waiter',
    waiterPlaceholder: 'Enter employee ID',
    waiterLogin: 'Enter',
    waiterLoggedIn: 'Current waiter',
    switchWaiter: 'Switch',
    selectTable: 'Select Table',
    table: 'Table',
    tablePlaceholder: 'Select table',
    addToCart: 'Add',
    cart: 'Cart',
    cartEmpty: 'Tap dishes on the left to add',
    cartItem: 'items',
    submitOrder: 'Submit Order',
    saveDraft: 'Save Draft',
    submitSuccess: 'Order submitted! Please pay at the counter.',
    draftSaved: 'Draft saved',
    clearCart: 'Clear',
    myOrders: 'My Orders',
    draftOrders: 'Drafts',
    pendingOrders: 'Pending',
    continueOrder: 'Continue',
    noDraft: 'No draft orders',
    noPending: 'No pending orders',
    orderSubmitted: 'Submitted',
    totalAmount: 'Total Amount',
    waiterRequired: 'Please login with a waiter account',
    loginSuccess: 'Login successful',
    loginRequired: 'Please log in with waiter account first',
    qty: 'Qty',
    flavorRequired: 'Please select',
    tableConflict: 'This table already has a pending order. Please complete payment first.',
  },

  admin: {
    title: 'Restaurant Management',
    pendingTitle: 'Pending Orders',
    pendingEmpty: 'No pending orders',
    checkout: 'Checkout',
    checkoutTitle: 'Payment',
    orderNo: 'Order No.',
    tableNo: 'Table',
    waiter: 'Waiter',
    amount: 'Amount',
    time: 'Time',
    paymentMethod: 'Payment Method',
    cash: 'Cash',
    pos: 'POS Machine',
    cashReceived: 'Amount Received',
    change: 'Change',
    quickRoundUp: 'Round Up',
    confirmCheckout: 'Confirm Payment',
    checkoutSuccess: 'Payment successful',
    receiptPrinted: 'Receipt printed',
    printFailed: 'Print failed: {error}',
    receiptTitle: 'Receipt',
    filterByWaiter: 'Filter by waiter',
    filterByTable: 'Filter by table',
    cancelOrder: 'Cancel Order',
    cancelOrderConfirm: 'Confirm cancel this order?',
    cancelDraftFailed: 'Failed to delete draft',
    cancelSuccess: 'Order cancelled',
    reopenOrder: 'Reopen Order',
    reopenSuccess: 'Order reopened',
    deleteOrder: 'Delete Order',
    deleteOrderConfirm: 'Confirm permanently delete this order? This action cannot be undone.',
    deleteSuccess: 'Order deleted',
  },

  kitchen: {
    addItems: 'Add Items',
    addItemsTitle: 'Add Items',
    cancelItem: 'Cancel Item',
    cancelItemTitle: 'Cancel Item',
    reprint: 'Reprint Kitchen Ticket',
    kitchenPrintOk: 'Kitchen print OK',
    kitchenPrintFail: 'Kitchen print failed',
    cancelReason: 'Reason',
    cancelReasonPlaceholder: 'Enter cancel reason (optional)',
    cancelQty: 'Cancel Qty',
    selectDish: 'Select Dish',
    qty: 'Qty',
    addToOrder: 'Add to Order',
    confirmAddItems: 'Confirm Add',
    confirmCancelItem: 'Confirm Cancel',
    addItemsSuccess: 'Items added',
    cancelItemSuccess: 'Item cancelled',
    reprintSuccess: 'Reprint sent',
    cancelled: 'Cancelled',
  },

  menu: {
    title: 'Menu Management',
    addDish: 'Add Dish',
    editDish: 'Edit Dish',
    dishName: 'Dish Name',
    dishCategory: 'Category',
    dishPrice: 'Price',
    dishImage: 'Image',
    dishStatus: 'Status',
    active: 'Active',
    inactive: 'Inactive',
    soldOut: 'Sold Out',
    moveUp: 'Move Up',
    moveDown: 'Move Down',
    sortOrder: 'Sort Order',
    addSuccess: 'Dish added',
    editSuccess: 'Dish updated',
    deleteSuccess: 'Dish deleted',
    deleteConfirm: 'Confirm delete this dish?',
    placeholderName: 'Enter dish name',
    placeholderNamePt: 'Enter Portuguese name',
    placeholderNameEn: 'Enter English name',
    placeholderCategory: 'Select or type category',
    placeholderPrice: 'Enter price',
    noDish: 'No dishes. Click Add to create.',
    manageCategory: 'Manage Categories',
    newCategoryPlaceholder: 'Enter new category name',
    noCategory: 'No categories yet',
    categoryNamePt: 'Portuguese Name',
    categoryNameEn: 'English Name',
    dishNamePt: 'Name (Portuguese)',
    dishNameEn: 'Name (English)',
    dishRemark: 'Remark',
    dishRemarkPt: 'Remark (Portuguese)',
    dishRemarkEn: 'Remark (English)',
    deleteCategoryConfirm: 'Confirm delete category "{name}"?',
    flavorTitle: 'Flavor Options',
    flavorEnabled: 'enabled',
    flavorRequired: 'Required',
    imageLimit: 'Maximum 4 images allowed',
    coverImage: 'Cover',
    categories: {
      hot: 'Hot Dishes',
      cold: 'Cold Dishes',
      staple: 'Staples',
      drink: 'Drinks',
      dessert: 'Desserts',
    },
  },

  history: {
    title: 'Order History',
    completedOrders: 'Completed Orders',
    orderCount: 'Order Count',
    totalSales: 'Total Sales',
    avgOrder: 'Avg. per Order',
    filterByWaiter: 'Waiter',
    filterByTable: 'Table',
    filterByPayment: 'Payment',
    exportExcel: 'Export Excel',
    noOrders: 'No completed orders',
  },

  reports: {
    title: 'Sales Reports',
    todayOverview: "Today's Overview",
    todayRevenue: "Today's Revenue",
    todayOrders: "Today's Orders",
    monthOverview: 'Monthly Overview',
    monthRevenue: 'Monthly Revenue',
    monthOrders: 'Monthly Orders',
    monthlyTrend: 'Monthly Sales Trend',
    categoryPie: 'Revenue by Category',
    paymentPie: 'Sales by Payment Method',
    dishRank: 'Top 10 Dishes',
    waiterRank: 'Waiter Performance',
    hourlyAnalysis: 'Orders by Hour',
    totalOrders: 'Total Orders',
    totalRevenue: 'Total Revenue',
    avgPerOrder: 'Avg. per Order',
    days: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
  },

  employee: {
    title: 'Employee Management',
    addEmployee: 'Add Employee',
    editEmployee: 'Edit Employee',
    employeeId: 'Employee ID',
    employeeName: 'Name',
    employeeRole: 'Role',
    employeeStatus: 'Status',
    waiter: 'Waiter',
    cashier: 'Cashier',
    admin: 'Administrator',
    active: 'Active',
    disabled: 'Disabled',
    addSuccess: 'Employee added',
    editSuccess: 'Employee updated',
    deleteSuccess: 'Employee deleted',
    defaultPassword: 'Default password: 123456',
    passwordHint: 'Leave blank to keep current password',
  },

  tableStatus: {
    available: 'Available',
    dining: 'Dining',
    pending: 'Pending',
  },

  receipt: {
    header: '========== RECEIPT ==========',
    footer: '========== THANK YOU! ==========',
    orderNo: 'Order No.',
    table: 'Table',
    waiter: 'Waiter',
    time: 'Time',
    payment: 'Payment',
    separator: '--------------------------',
    total: 'Total',
    received: 'Received',
    change: 'Change',
    thankYou: 'Thank you, come again!',
  },
}
